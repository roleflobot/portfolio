...
export default function AiComment({ id, comment, onChange }: AiCommentProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'AI 한줄평 생성에 실패했습니다.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div>
      {comment && (
        <p className="text-wood-ink italic mb-2">&ldquo;{comment}&rdquo;</p>
      )}
      ...
      >
        {loading ? '작성 중...' : comment ? 'AI 한줄평 다시 쓰기' : 'AI 한줄평 쓰기'}
      </button>
    </div>
  )
}
